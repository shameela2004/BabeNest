// this is the before connecting with the backend 
...
//         {cartItems.map(item => (
//           <div key={item.id} className="flex items-center gap-4 border rounded p-4">
//             <img src={item.image} alt={item.product.name} className="w-24 h-24 object-cover rounded" />
//             <div className="flex-1">
    ...
    <>
      ...
      <div className="max-w-5xl mx-auto p-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <button
                    ...
                    onClick={() => updateQuantity(item.id, item.quantity - 1)}
                    disabled={item.quantity === 1}
                    className="text-2xl font-bold text-gray-700 hover:text-gray-900 disabled:opacity-40"
                  >
                    -
                  </button>
                  <div className="border rounded px-3">{item.quantity}</div>
                  <button
                    type="button"
                    onClick={() => updateQuantity(item.id, item.quantity + 1)}
        ...
        <div className="mt-6 flex justify-between items-center font-semibold text-xl">
          ...
          <button
            onClick={clearCart}
            className="flex items-center gap-2 text-white bg-gray-600 hover:bg-gray-800 px-4 py-2 rounded"
          >
            <BiTrash size={20} /> Clear Cart
          </button>
        </div>

        <Link
          to="/checkout"
          className="inline-block mt-6 bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded text-center text-lg"
        >
          Proceed to Checkout
        </Link>
      </div>
