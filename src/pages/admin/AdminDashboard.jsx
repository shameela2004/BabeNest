...
orders.forEach((o) => {
  if (o.paymentStatus?.toLowerCase() === "paid") {
    ...
    monthlyRevenue[month] += Number(o.totalAmount) || 0;
  }
});
    ...
    <div className="p-6 space-y-8 max-w-screen-xl mx-auto">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        <StatCard
          ...
          label="Total Revenue"
          value={`₹${totalRevenue}`}
          color="bg-pink-100 text-pink-700 col-span-full"
        />
      </div>
...
const StatCard = ({ icon, label, value, color }) => (
  <div
    className={`p-4 rounded-lg shadow-md flex items-center justify-between ${color}`}
  >
    <div className="text-2xl">{icon}</div>
    <div className="text-right">
      <div className="text-sm">{label}</div>
