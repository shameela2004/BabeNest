//correct user mangement page before connectig with the backend
...
function ManageUsers() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await api.get('/AdminUser', { params: queryParams });

      // Fix: access actual array of users and total count
      const usersData = res?.data?.data?.data ?? [];
      const total = res?.data?.data?.totalCount ?? 0;

      // Optional: exclude admin accounts
